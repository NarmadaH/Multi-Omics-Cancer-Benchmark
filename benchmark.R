...
get.clustering.results.dir.path <- function() {
  return("./Multi-Omics-Cancer-Benchmark/All_Results")
}
...
ALGORITHM.NAMES <- c(
  # "kmeans", "spectral", "lracluster", "pins", "snf", "mkl",
  # "mcca", "nmf", "iCluster"
  "kmeans"
)
ALGORITHM.DISPLAY.NAMES <- as.list(c(
  # "K-means", "Spectral", "LRAcluster", "PINS",
...
perform.all.analyses <- function(benchmark.ret) {
  ...
  for (i in 1:4) {
    cur.func <- list(
      benchmark.omics.time, benchmark.omics.num.clusters,
      benchmark.omics.surv, benchmark.omics.clinical
    )[[i]]
    for (omic.subset in names(OMIC.SUBSETS)) {
      ...
      colnames(displayed.benchmark.data)[1:ncol(displayed.benchmark.data)] <-
        sapply(
          as.list(colnames(displayed.benchmark.data)[1:ncol(displayed.benchmark.data)]),
  ...
  for (i in 1:4) {
    ...
    plot.name <- list(
      "multi_omics_surv_clinical.tiff", "exp_surv_clinical.tiff",
      "methy_surv_clinical.tiff", "mirna_surv_clinical.tiff"
    )[[i]]
...
create.mean.clinical.survival.plot <- function(benchmark.ret) {
  ...
  for (i in 2:6) {
    ...
    subplot.name <- c(
      "Multi-omics", "Gene Expression", "DNA Methylation", "miRNA Expression",
      "Best single-omic (survival)", "Best single-omic (clinical)"
    )[i]
    plot(surv.sum, clin.sum,
      main = subplot.name, xlab = xlab, ylab = ylab,
...
#     plot(subtype.surv, subtype.clinical,
#       main = subtype.to.display.name(subtype), xlab = xlab, ylab = ylab,
...
create.clinical.survival.plots <- function(benchmark.surv, benchmark.clinical, plot.name) {
  ...
  num.rows <- 2
  num.cols <- ceiling(num.subtypes / num.rows)

  par(mfrow = c(num.rows, num.cols), mar = c(4.1, 4, 3.2, 1))
  for (i in 1:num.subtypes) {
    subtype <- colnames(benchmark.surv)[i]
    subtype.surv <- benchmark.surv[, subtype]
    subtype.clinical <- benchmark.clinical[, subtype]
...
analyze.benchmark <- function() {
  print ("START analyze.benchmark")
  all.clusterings <- list()
  all.timings <- list()
  for (i in 1:length(SUBTYPES.DATA)) {
    current.subtype.data <- SUBTYPES.DATA[[i]]
    print(paste0("current subtype ", current.subtype.data$name))
    ...
    for (algorithm.name in ALGORITHM.NAMES) {
      print(paste0("current algorithm ", algorithm.name)  )

      all.clusterings[[subtype]][[algorithm.name]] <- list()
      all.timings[[subtype]][[algorithm.name]] <- list()
      for (j in c("all", "1", "2", "3")) {
        clustering.path <- file.path(
          get.clustering.results.dir.path(),
          paste(subtype, algorithm.name, j, sep = "_")
        )
        print(paste0("clustering path ", clustering.path))

        timing.path <- file.path(
          get.clustering.results.dir.path(),
          paste(subtype, algorithm.name, j, "timing", sep = "_")
        )
        print(paste0("timing path ", timing.path))
        ...
        print(paste0("clustering length ", length(clustering)))
        print(paste0("timing length ", length(timing)))

        print ("DONE analyze.benchmark")
      }
    }
  }
...
#   return(list(
#     pvalue = cur.pvalue, conf.int = cur.conf.int, total.num.perms = total.num.perms,
#     total.num.extreme.chisq = total.num.extreme.chisq
#   ))
# }


# for windows - no parallel processing
...
get.empirical.surv <- function(clustering, subtype) {
  ...
  while (should.continue) {
    ...
    perm.chisq <- numeric(num.perms)
    for (i in 1:num.perms) {
      cur.clustering <- sample(clustering)
      names(cur.clustering) <- names(clustering)
      cur.chisq <- check.survival(cur.clustering, subtype)$chisq
      perm.chisq[i] <- cur.chisq
    }
...
run.benchmark <- function() {
  ...
  for (i in 1:length(SUBTYPES.DATA)) {
    ...
    print(paste0("current subtype ", subtype))

    subtype.raw.data <- get.raw.data(subtype,
      only.primary = current.subtype.data$only.primary
    )

    subtype.raw.data <- set.omics.list.attr(
      subtype.raw.data,
    ...
    for (algorithm.name in ALGORITHM.NAMES) {
      for (j in c("all", "1", "2", "3")) {
        ...
        print(paste("clustering path", clustering.path))
        ...
        print(paste("timing path", timing.path))


        if (!file.exists(clustering.path)) {
          print(paste("file doesnt exist", clustering.path))
          ...
          print(paste("before running", algorithm.name))
          ...
          # write.csv(clustering, file = clustering.path, row.names = FALSE)
          # write.csv(timing, file = timing.path, row.names = FALSE)

          # Assuming clustering and timing are matrices
          # write.table(clustering, file = clustering.path, row.names = FALSE, col.names = TRUE, sep = "\t")
          # write.table(timing, file = timing.path, row.names = FALSE, col.names = TRUE, sep = "\t")
...
fix.patient.names <- function(subtype.raw.data, include.type = F) {
  for (i in 1:length(subtype.raw.data)) {
    colnames(subtype.raw.data[[i]]) <- get.fixed.names(
      colnames(subtype.raw.data[[i]]),
      include.type
    )
  }
  return(subtype.raw.data)
...
get.raw.data <- function(subtype.name,
                         ...
                         only.primary = NA) {
  print(paste0("get.raw.data ", subtype.name))

  omics.dir <- file.path(datasets.path, subtype.name, "/")
  print(paste0("get.raw.data file path ", omics.dir))

  # Check if the directory exists
  if (dir.exists(omics.dir)) {
...
run.snf <- function(omics.list, subtype.data) {
  ...
  similarity.data <- lapply(omics.list, function(x) {
    affinityMatrix(
      dist2(as.matrix(t(x)), as.matrix(t(x))),
      num.neighbors, alpha
    )
  })
...
run.iCluster <- function(omics.list, subtype.data) {
  ...
  if (length(omics.list) == 1) {
    icluster.ret <- iClusterPlus::tune.iClusterBayes(
      cpus = (MAX.NUM.CLUSTERS - 1), t(omics.list[[1]]),
      K = 1:(MAX.NUM.CLUSTERS - 1), type = c("gaussian")
    )$fit
  } else {
    icluster.ret <- iClusterPlus::tune.iClusterBayes(
      cpus = (MAX.NUM.CLUSTERS - 1), t(omics.list[[1]]),
      t(omics.list[[2]]),
      t(omics.list[[3]]),
      K = 1:(MAX.NUM.CLUSTERS - 1), type = rep("gaussian", 3)
    )$fit
  }
  ...
  time.taken <- as.numeric(Sys.time() - start, units = "secs")
  return(list(
    clustering = optimal.solution$clusters,
    timing = time.taken
  ))
}
...
run.mkl <- function(omics.list, subtype.data) {
  ...
  command <- paste(
    bin.path, paste0(subtype.dir, "kernels"),
...
run.pins <- function(omics.list, subtype.data) {
  ...
  omics.transposed <- lapply(omics.list, t)
  if (length(omics.list) == 1) {
    pins.ret <- PINSPlus::PerturbationClustering(
      data = omics.transposed[[1]],
      kMax = MAX.NUM.CLUSTERS
    )
    clustering <- pins.ret$cluster
  } else {
    pins.ret <- PINSPlus::SubtypingOmicsData(
      dataList = omics.transposed,
      kMax = MAX.NUM.CLUSTERS
    )
...
run.kmeans <- function(omics.list, subtype.data) {
  ...
  time.taken <- as.numeric(Sys.time() - start, units = "secs")
  return(list(
    clustering = all.clusterings[[best.k]],
    timing = time.taken
  ))
}
...
run.spectral <- function(omics.list, subtype.data) {
  ...
  similarity.data <- affinityMatrix(
    dist2(
  ...
  num.clusters <- estimateNumberOfClustersGivenGraph(
    similarity.data,
    2:MAX.NUM.CLUSTERS
  )[[3]]
...
export.subtype.to.mkl <- function(omics.list, dir.name) {
  ...
  write.table(colnames(omics.list[[1]]),
    file = file.path(folder.path, "ids"),
...
check.clinical.enrichment <- function(clustering, subtype.name) {
  ...
  clinical.metadata <- list(
    gender = "DISCRETE", age_at_initial_pathologic_diagnosis = "NUMERIC",
